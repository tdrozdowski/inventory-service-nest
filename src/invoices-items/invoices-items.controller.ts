...
export class InvoicesItemsController {
  ...
  async findOne(
    ...
    const invoiceItem = await this.invoicesItemsService.findOne(
      invoiceId,
      itemId,
    );
    if (!invoiceItem) {
      throw new NotFoundException(
        `Invoice item with invoice_id ${invoiceId} and item_id ${itemId} not found`,
      );
    }
    return invoiceItem;
  }
  ...
  @ApiOperation({
    ...
    description:
      'Deletes a specific association between an invoice and an item',
  })
  @ApiParam({ name: 'invoiceId', description: 'ID of the invoice' })
  @ApiParam({ name: 'itemId', description: 'ID of the item' })
  @ApiResponse({
